...
if __name__ == '__main__':
    ...
    for epoch in range(opt.n_epochs):
        ...
        # for i in range(12):
            ...
            """
            net(gen_imgs.detach()) 训练student网络
            计算Student网络与Teacher网络输出的KD散度
            """
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(data_test_loader):  # test测试阶段需要测试集
                images = images.cuda()
                labels = labels.cuda()
                net.eval()
                # output = net(images)
        ...
        if accr > accr_best:  # 如果是当前最好的精度，保存模型
